...
over(ORDER BY event_date ROWS BETWEEN 49 PRECEDING AND CURRENT ROW) AS moving_average_last_50_transaction
FROM pricedata; 
 ...
 SELECT DAYNAME(event_date), COUNT(*) AS sales, AVG(eth_price) AS average_eth_price
 FROM pricedata
...
SELECT CONCAT( name," was sold for ", ROUND(usd_price,-3), " to ", seller_address, " from ", buyer_address," on ", event_date) AS each_sale
FROM pricedata;
...
SELECT CONCAT(ROUND(eth_price,-2), " - ", ROUND(eth_price,-2)+99) AS price_range, COUNT(*),
RPAD('',COUNT(*),'*')
FROM pricedata
...
SELECT CONCAT(MONTHNAME(event_date)," ",YEAR(event_date)) AS month_year, ROUND(SUM(usd_price), -2) AS total_volumne
FROM pricedata
GROUP BY month_year
ORDER BY total_volumne DESC;

/*12.	Count how many transactions the wallet "0x1919db36ca2fa2e15f9000fd9cdc2edcf863e685"had over this time period.*/

SELECT COUNT(*) AS wallet
FROM pricedata
...
CREATE TEMPORARY TABLE average_USD_price_for_eachday
SELECT event_date, usd_price, AVG(usd_price) OVER(PARTITION BY event_date ORDER BY event_date) as average_usd_by_day
FROM pricedata; 
 
SELECT *, AVG(usd_price) OVER(partition by event_date) AS new_estimated_daily_average
FROM average_USD_price_for_eachday 
WHERE usd_price > 0.1*average_usd_by_day;
...
SELECT
    wallet_address,
    SUM(CASE WHEN is_buyer = 1 THEN -usd_price ELSE usd_price END) AS total_profit_loss
FROM
    (
        SELECT buyer_address AS wallet_address, usd_price, 1 AS is_buyer
        FROM pricedata
        UNION ALL
        SELECT seller_address AS wallet_address, usd_price, 0 AS is_buyer
        FROM pricedata
    ) AS combined_data
GROUP BY
    wallet_address
